...
async def space_track_cdm_diagnostic_test():
    ...
    Deliberately minimal: one login, one small query (limit 5), same
    restrained approach as the other Space-Track diagnostics in this
    file. The exact query shape is reconstructed from general knowledge
    of Space-Track's documented API, not independently verified live
    (space-track.org isn't reachable from the development environment
    this was built in) \u2014 real behavior is only confirmed once this
    actually runs against real, logged-in credentials on Render.
    """
    if not SPACE_TRACK_USERNAME or not SPACE_TRACK_PASSWORD:
        return {
            "status": "not_configured",
    ...
    login_url = "https://www.space-track.org/ajaxauth/login"
    cdm_query_url = "https://www.space-track.org/basicspacedata/query/class/cdm/orderby/CREATED%20desc/limit/5/format/json"
    ...
    try:
        async with httpx.AsyncClient(timeout=20.0) as client:
            ...
            cdm_resp = await client.get(cdm_query_url)

            if cdm_resp.status_code == 403:
                return {
                    "status": "no_access",
                    "detail": "Login succeeded, but the CDM class returned HTTP 403 \u2014 this account does not have permission to query conjunction data messages.",
                    "stage": "cdm_query",
                    "http_status": 403,
                    "conclusion": "Real Pc engine (Phase 2) is blocked on account access, not a technical build problem. Would need to contact Space-Track about registering as an owner/operator, or confirm whether a different access tier is required.",
                }

            if cdm_resp.status_code != 200:
                return {
                    "status": "error",
                    "detail": f"Login succeeded, but the CDM query failed: HTTP {cdm_resp.status_code}",
                    "stage": "cdm_query",
                    "http_status": cdm_resp.status_code,
                    "response_body_snippet": cdm_resp.text[:500],
                }
            ...
            try:
                ...
                return {
                    "status": "error",
                    "detail": "CDM query returned HTTP 200 but the body wasn't valid JSON \u2014 possibly an HTML login/error page instead of real data.",
                    "stage": "cdm_query",
                    "response_body_snippet": cdm_resp.text[:500],
                }
            ...
            if isinstance(data, list) and len(data) == 0:
                return {
                    "status": "empty",
                    "detail": "Login and query both succeeded (HTTP 200), but zero CDM records were returned.",
                    "conclusion": "This could mean the account has access but there's simply no current conjunction data for whatever this account is scoped to see \u2014 or it could mean access is restricted to specific registered objects this account doesn't have any registered for. Real query worked; the emptiness itself doesn't distinguish between those two honestly without a follow-up test against a known-active object.",
                }

            return {
                "status": "success",
                "detail": f"Real login and real CDM query both succeeded, {len(data) if isinstance(data, list) else '?'} record(s) returned.",
                "conclusion": "This account DOES have some real access to CDM data. Next real step would be inspecting the actual fields present (especially covariance data) to confirm whether it's enough to build genuine Pc, not just confirm access exists.",
                "sample_data": data,
            }
