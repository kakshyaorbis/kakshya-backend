...
def celestrak_health():
    ...
    exactly one row per category \u2014 its most recent real status). Fixed
    after a real, honest bug caught directly by the user: the original
    version only looked at the single most-recently-checked category,
    which could show "fully healthy" even while another category was
    actively on the Space-Track failover, just because it happened to be
    checked slightly earlier. This version can't hide a partial fallback
    that way.
    """
    db = SessionLocal()
    try:
        ...
        healthy_count = sum(1 for e in all_entries if not e.is_error)
        total_checked = len(all_entries)
        unhealthy_categories = sorted(e.group_name for e in all_entries if e.is_error)
