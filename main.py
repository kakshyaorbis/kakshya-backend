...
@app.get("/api/debug/space-track-cdm-test")
async def space_track_cdm_diagnostic_test():
    """
    DIAGNOSTIC ONLY \u2014 checks whether this Space-Track account actually
    has access to CDM (Conjunction Data Message) data, the real
    covariance/uncertainty data a genuine collision-probability (Pc)
    engine would need. This does NOT build Pc itself \u2014 it only answers
    the access question first, honestly, before any further work is
    scoped on top of it.

    UPDATE after first real run: the initial guess at the API class name
    ("cdm") was confirmed WRONG \u2014 Space-Track returned "Your Class Does
    Not Exist". Login itself succeeded, so credentials are genuinely
    valid; only the specific class name was the problem. This version
    tries a short, bounded list of plausible alternate names in one
    login session rather than guessing once per redeploy. Every name
    here is still a reconstructed guess from general knowledge, not
    verified against Space-Track's actual current documentation \u2014 the
    one fully authoritative source is Space-Track's own API docs, viewed
    directly while logged into their site, since this environment has no
    live access to space-track.org to check that first.

    Real, documented uncertainty going in: Space-Track has historically
    scoped full CDM access (with covariance) to registered satellite
    owner/operators for THEIR OWN objects, not as a general-purpose feed
    any account can query for arbitrary objects. Whether that's still
    true, and whether it applies to this account, is exactly what this
    test is for \u2014 not assumed either way beforehand.

    Deliberately bounded: one login, reused for a small handful of
    candidate-class queries (limit 5 each), same restrained approach as
    the other Space-Track diagnostics in this file.
    """
    if not SPACE_TRACK_USERNAME or not SPACE_TRACK_PASSWORD:
        return {
            "status": "not_configured",
    ...
    login_url = "https://www.space-track.org/ajaxauth/login"
    # Ordered by confidence, highest first. "cdm" already confirmed wrong
    # in a real run \u2014 kept out of this list on purpose, no point retrying
    # a class name Space-Track has already told us doesn't exist.
    candidate_classes = ["cdm_public", "conjunction_data_message", "cdm_claim"]
    ...
    try:
        async with httpx.AsyncClient(timeout=20.0) as client:
            ...
            for class_name in candidate_classes:
                query_url = f"https://www.space-track.org/basicspacedata/query/class/{class_name}/orderby/CREATED%20desc/limit/5/format/json"
                resp = await client.get(query_url)

                if resp.status_code == 403:
                    attempts.append({"class_name": class_name, "result": "no_access", "http_status": 403})
                    continue

                if resp.status_code != 200:
                    attempts.append({
                        "class_name": class_name, "result": "error",
                        "http_status": resp.status_code, "response_body_snippet": resp.text[:300],
                    })
                    continue
                ...
                try:
                    ...
                    attempts.append({"class_name": class_name, "result": "invalid_json", "response_body_snippet": resp.text[:300]})
                    continue

                if isinstance(data, list) and len(data) == 0:
                    attempts.append({"class_name": class_name, "result": "valid_class_but_empty"})
                    continue

                # A real, working class name that returned real data \u2014
                # stop here, no need to try the remaining candidates.
                return {
                    "status": "success",
                    "detail": f'Class name "{class_name}" is real and returned {len(data) if isinstance(data, list) else "?"} record(s).',
                    "conclusion": "This account DOES have some real access to CDM-type data under this class name. Next real step: inspect the actual fields present (especially covariance data) to confirm whether it's enough to build genuine Pc, not just confirm access exists.",
                    "working_class_name": class_name,
                    "attempts_before_success": attempts,
                    "sample_data": data,
                }

            # None of the candidates worked cleanly.
            any_valid_but_empty = any(a["result"] == "valid_class_but_empty" for a in attempts)
            return {
                "status": "empty_or_unresolved",
                "detail": "None of the candidate class names returned real data.",
                "attempts": attempts,
                "conclusion": (
                    "At least one candidate class name is valid but returned zero records \u2014 access may exist but scoped to specific registered objects this account has none of, OR the real class name is still one we haven't tried."
                    if any_valid_but_empty else
                    "None of the guessed class names exist on Space-Track's side. This needs the real class name from Space-Track's own API documentation \u2014 worth checking directly on space-track.org while logged in, since this environment can't reach that site to look it up."
                ),
            }
