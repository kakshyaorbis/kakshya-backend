...
async def celestrak_group(group: str):
    ...
    try:
        ...
        if failover_ok:
            save_cache(failover_data, False)
            return PlainTextResponse(
        ...
        # substitute.
        save_cache(celestrak_error, True)
        return JSONResponse(status_code=502, content={
...
def celestrak_health():
    """
    Real, lightweight Celestrak status check for the dashboard's health
    indicator \u2014 deliberately reads from the EXISTING cache data (already
    being collected by every real /api/celestrak/{group} call) rather than
    triggering any new request of its own. Zero additional load on
    Celestrak, no matter how often the dashboard polls this.

    Reports based on whichever category was most recently actually
    checked \u2014 a real, current signal, not a guess.
    """
    db = SessionLocal()
    try:
        latest = db.query(CelestrakCacheEntry).order_by(CelestrakCacheEntry.cached_at.desc()).first()
        if not latest:
            return {"celestrak_healthy": None, "detail": "No category has been checked yet since this server last restarted"}
        ...
        return {
            "celestrak_healthy": not latest.is_error,
            "last_checked_utc": cached_at.isoformat(),
            "checked_category": latest.group_name,
        }
    finally:
        db.close()
