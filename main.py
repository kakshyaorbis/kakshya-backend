...
async def space_track_category(category: str):
    """
    ...
    db = SessionLocal()
    try:
        cached = db.query(SpaceTrackCacheEntry).filter(SpaceTrackCacheEntry.category_name == category).first()
        if cached:
            cached_at = cached.cached_at
            if cached_at.tzinfo is None:
                cached_at = cached_at.replace(tzinfo=timezone.utc)
        ...
        try:
            async with httpx.AsyncClient(timeout=25.0) as client:
                ...
                return JSONResponse(content=data, headers={"X-Cache": "miss", "X-Source": "space-track"})
