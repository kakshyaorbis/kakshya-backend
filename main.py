...
async def load_tracked_satellites():
    ...
    real space stations) once at startup, using the same real Celestrak
    endpoint and headers already proven in /api/celestrak/{group}.
    """
    try:
        ...
        if resp.status_code != 200 or not resp.text.strip():
            logger.warning(f"Could not load real satellite data for WebSocket tracking: HTTP {resp.status_code}")
            return
        ...
        for i in range(0, len(lines) - 2, 3):
            ...
            if count >= 8:  # a small, real, manageable set for this demo — not the full stations catalog
                break
        logger.info(f"Loaded {count} real satellites for WebSocket tracking (Celestrak 'stations' group)")
    except Exception as e:
        logger.warning(f"Failed to load real satellite data for WebSocket tracking: {e}")
