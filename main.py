...
def login(req: LoginRequest, response: Response):
    """
    Checks real credentials against the database, and — new in this
    version — issues a real signed session token as an HttpOnly cookie on
    success. HttpOnly means client-side JavaScript can never read or
    tamper with it, which is what makes this a real session rather than
    just a client-side "remember I logged in" flag.
    """
    db = SessionLocal()
    try:
        ...
        response.set_cookie(
            ...
            value=token,
            httponly=True,
            secure=True,       # only sent over HTTPS — Render serves everything over HTTPS by default
            samesite="lax",
