...
async def load_tracked_satellites():
    ...
    The initial 10-second delay below is not a guess — it's a direct
    response to real evidence: live Render logs showed all 3 retry
    attempts failing with ConnectTimeout specifically when this ran
    immediately at cold-start, while the same URL/headers/logic already
    work fine on /api/celestrak/{group}, which only ever runs in response
    to a normal user request (never at the exact moment of startup). The
    most likely real cause is the container's outbound networking not
    being fully ready in the first few seconds after waking from a
    free-tier spin-down — this delay gives that time to actually happen
    before the first real attempt, instead of retrying too close together
    during the exact window when it's failing.
    """
    # First attempt at 10s wasn't enough — live Render logs showed BOTH
    # retries still timing out at the full 15s each, suggesting a longer
    # outbound-networking warm-up window after a free-tier cold start than
    # initially estimated. Raised to 45s as a real improvement, but the
    # actual guarantee is the 5-minute background safety net below, not
    # this specific number — on a free-tier service waking from a full
    # spin-down, it's genuinely normal for real data to take a few minutes
    # to first appear, not a bug to keep chasing with ever-more-precise
    # delay tuning.
...
async def start_background_tasks():
    # load_tracked_satellites() runs as a background task, NOT awaited
    # directly here — its retry delays (up to ~12s total) would otherwise
    # block the entire app from finishing startup and serving ANY request
    # (even unrelated ones like /health) until all retries completed. That
    # was a real bug caught during this exact test: Render (or any health
    # check) could reasonably conclude the deploy was unhealthy during
    # that window. The app is now fully ready immediately; satellite
    # tracking data simply arrives a few seconds later in the background.
