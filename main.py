...
async def space_track_bulk_diagnostic_test():
    ...
    query_url = "https://www.space-track.org/basicspacedata/query/class/gp/OBJECT_NAME/~~STARLINK/orderby/NORAD_CAT_ID/limit/5/format/json"
...
async def space_track_scale_diagnostic_test():
    ...
    try:
        async with httpx.AsyncClient(timeout=25.0) as client:
            ...
            for limit in tiers:
                query_url = f"https://www.space-track.org/basicspacedata/query/class/gp/OBJECT_NAME/~~STARLINK/orderby/NORAD_CAT_ID/limit/{limit}/format/json"
                try:
                    data_resp = await client.get(query_url)
                    if data_resp.status_code != 200:
...
async def space_track_category(category: str, refresh: bool = False):
    ...
    try:
        ...
        login_url = "https://www.space-track.org/ajaxauth/login"
        query_url = f"https://www.space-track.org/basicspacedata/query/class/gp/OBJECT_NAME/~~{name_pattern}/orderby/NORAD_CAT_ID/limit/{SPACE_TRACK_RESULT_CAP}/format/json"
    ...
    try:
        async with httpx.AsyncClient(timeout=25.0) as client:
            ...
            for celestrak_name, query_type, value, confidence in tests:
                if query_type == "name":
                    query_url = f"https://www.space-track.org/basicspacedata/query/class/gp/OBJECT_NAME/~~{value}/orderby/NORAD_CAT_ID/limit/5/format/json"
                else:  # period_range
                    low, high = value
                    query_url = f"https://www.space-track.org/basicspacedata/query/class/gp/PERIOD/{low}--{high}/orderby/NORAD_CAT_ID/limit/5/format/json"
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        results[celestrak_name] = {
                            ...
                            "sample_names": [o.get("OBJECT_NAME") for o in active[:5]],
                        }
                    else:
