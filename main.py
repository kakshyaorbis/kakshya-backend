...
async def try_stations_space_track_failover():
    ...
    Deliberately, honestly limited to ISS only right now \u2014 the one
    object confidently verified throughout this whole investigation.
    Extend this dict only once another station's ID is actually
...
async def try_space_track_failover(group: str) -> tuple[bool, str]:
    ...
    if mapping["type"] == "name":
        ...
        # Adds OBJECT_TYPE/PAYLOAD to exclude debris entries that a plain
        # name search picks up \u2014 verified via real testing to cleanly
        # remove "DEB" entries for weather/NOAA specifically. Does NOT
        # exclude old-but-still-technically-a-payload objects; only used
        # for categories where this was directly confirmed sufficient.
