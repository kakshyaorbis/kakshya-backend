...
async def n2yo_category_sweep():
    """
    DIAGNOSTIC ONLY \u2014 tests a real range of N2YO category IDs (0\u201330)
    ...
    Deliberately conservative: a 0.5s delay between each of the 31 real
    requests (this alone takes ~15+ seconds to complete), and the result
    ...
    final = {
        "detail": "Real results for category IDs 0\u201330, from a live sweep against New Delhi \u2014 use the sample_names to identify which ID (if any) maps to something meaningful like 'Starlink' or 'weather'",
