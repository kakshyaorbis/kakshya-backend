    ...
    for attempt in range(1, 3):
        ...
        try:
            ...
            if resp.status_code in (403, 404):
                last_error = f"HTTP {resp.status_code} (explicit rejection — not retrying, per Celestrak's own stated policy)"
                satellite_load_status["last_error"] = last_error
                logger.warning(f"Celestrak explicitly rejected the request: {last_error}. Stopping immediately, no retry.")
...
# Real, server-side cache — {group: {"data": str, "cached_at": datetime,
# "is_error": bool}}. The existing Cache-Control header only ever protected
# against repeat hits from the SAME browser tab; it did nothing against
# curl/diagnostic tools, multiple different users, or automated monitors,
# all of which bypass client-side caching entirely. This is the real fix:
# no matter how many times or from where this endpoint is called, Celestrak
# itself is only actually contacted once per cache window.
celestrak_cache: dict = {}
...
async def celestrak_group(group: str):
    ...
    whitelist and identifying header as before, now with a REAL server-side
    cache (not just a client-side header) so repeated calls from any
    source — browser, curl, a monitoring script, multiple different users —
    only ever result in Celestrak actually being contacted once per cache
    window, never more.
    """
    ...
    now = datetime.now(timezone.utc)
    cached = celestrak_cache.get(group)
    if cached:
        ...
        # repeated hits during an outage or block, short enough to recover
        # reasonably once Celestrak stabilizes, without needing a manual
        # restart to clear a stale failure.
        cache_window = 14400 if not cached["is_error"] else 3600
        if age_seconds < cache_window:
            if cached["is_error"]:
                return JSONResponse(status_code=502, content={"error": cached["data"], "note": "cached failure — not re-attempted yet, to avoid repeatedly hitting Celestrak"})
    ...
    try:
        ...
        if resp.status_code != 200:
            ...
            celestrak_cache[group] = {"data": error_msg, "cached_at": now, "is_error": True}
        ...
        celestrak_cache[group] = {"data": resp.text, "cached_at": now, "is_error": False}
